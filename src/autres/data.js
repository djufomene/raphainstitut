import home1 from '../assets/images/home1.jpg';
import home2 from '../assets/images/home2.jpg';
import home3 from '../assets/images/home3.jpg';
import home4 from '../assets/images/home4.jpg';
import home5 from '../assets/images/home5.jpg';
import home6 from '../assets/images/home6.png';
import home7 from '../assets/images/home7.jpg';
import home8 from '../assets/images/home8.jpg';
import home9 from '../assets/images/home9.jpg';
import home10 from '../assets/images/home10.jpg';
import home11 from '../assets/images/home11.png';
import home12 from '../assets/images/home12.png';
import home13 from '../assets/images/home13.png';
import home14 from '../assets/images/home14.png';
import home15 from '../assets/images/home15.png';
import home16 from '../assets/images/home16.jpg';
import home17 from '../assets/images/home17.jpg';
import home20 from '../assets/images/home20.png';
import home21 from '../assets/images/home21.png';
import home22 from '../assets/images/home22.png';
import home23 from '../assets/images/home23.jpg';
import home24 from '../assets/images/home24.jpg';
import home25 from '../assets/images/home25.jpg';
import home26 from '../assets/images/home26.jpg';
import home27 from '../assets/images/home27.jpg';
import home28 from '../assets/images/home28.jpg';
import home29 from '../assets/images/home29.jpg';
import home30 from '../assets/images/home30.jpg';
import home31 from '../assets/images/home31.jpg';
import home32 from '../assets/images/home32.jpg';
import home33 from '../assets/images/home33.jpg';
import home34 from '../assets/images/home34.jpg';
import home35 from '../assets/images/home35.jpg';
import home36 from '../assets/images/home36.png';
import home37 from '../assets/images/home37.jpg';
import home38 from '../assets/images/home38.jpg';
import home39 from '../assets/images/home39.jpg';
import home40 from '../assets/images/home40.png';
import home41 from '../assets/images/home41.jpg';

export const images = {
   home1,
   home2,
   home3,
   home4,
   home5,
   home6,
   home7,
   home8,
   home9,
   home10,
   home11,
   home12,
   home13,
   home14,
   home15,
   home16,
   home17,
   home20,
   home21,
   home22,
   home23,
   home24,
   home25,
   home26,
   home27,
   home28,
   home29,
   home30,
   home31,
   home32,
   home33,
   home34,
   home35,
   home36,
   home37,
   home38,
   home39,
   home40,
   home41,
};



// src/autres/dataLabo.js
export const labos = [
 
   {
     image: 'home28',
     titre: 'Microscope optique binoculaire',
     texte: "Le microscope est un outil fondamental en biologie médicale. Il permet l’observation de cellules, bactéries, parasites et autres micro-organismes invisibles à l'œil nu. il Initie les étudiants aux techniques de microscopie essentielles pour le diagnostic biologique, Développe la rigueur dans l’analyse et la traçabilité des résultats et Permet d'appliquer les connaissances théoriques en cytologie, hématologie ou microbiologie.",
   },
 
   {
     image: 'home30',
     titre: 'Appareil d’échographie (échographe)',
     texte: 'Un échographe est un appareil médical qui permet de visualiser l’intérieur du corps humain grâce à des ultrasons. C’est l’appareil utilisé pour réaliser une échographie. Il Initie les étudiants en imagerie médicale aux techniques d’exploration non invasives, Permet de comprendre l’anatomie fonctionnelle en visualisant directement les organes en mouvement, Prépare les apprenants aux contextes cliniques réels en pédiatrie, gynécologie, ou médecine générale.',
   },
   {
     image: 'home31',
     titre: "Salle d'hospitalisation et Mannequin médical de simulation haute fidélité",
     texte: "Salle équipé des lits et des mannequins grandeur nature utilisé pour la simulation de soins. C'est un Outil clé dans la formation aux gestes techniques (pose de perfusion, massage cardiaque, prise de tension, etc.), Permet une immersion sécurisée dans des situations d'urgence sans risque pour un vrai patient, Développe la coordination, la réactivité et la confiance des étudiants en milieu médical simulé.",
   },
 ];



 // src/autres/data.js
export const formationData = {
  "Licences Professionnelles": {
  title: "Licences Professionnelles",
  options: [
    "Ressources humaines",
    "Commerce international",
    "Gestion des entreprises",
    "Informatique de gestion",
    "Communication",
    "Marketing digital"
  ]
},

  "Sciences Médico-sanitaire": {
    title: "Sciences Médico-sanitaire",
    options: [
      "Science infirmière",
      "Kinésithérapie",
      "Sage-femme / Maïeuticien",
      "Analyse médicale et technique de laboratoire",
      "Imagerie médicale et radiologie",
      "Science pharmaceutique",
      "Odontostomatologie",
      "Délégué médical",
      "Auxiliaire de vie",
      "Auxiliaire de pharmacie"
    ]
  },
  "Génie Informatique": {
    title: "Génie Informatique",
    options: [
      "Gestion du système informatique",
      "Infographie et web design"
    ]
  },
  "Science et Gestion": {
    title: "Science et Gestion",
    options: [
      "Assurance",
      "Banque",
      "Commerce international",
      "Communication des organisations",
      "Comptabilité informatisée de gestion",
      "Douane et transit",
      "Droit des affaires et de l'entreprise",
      "Gestion de la qualité",
      "Gestion fiscale",
      "Logistique et transport",
      "Marketing commerce vente",
      "Ressources humaines"
    ]
  },

  "Génie des Télécommunications et Réseaux": {
    title: "Génie des Télécommunications et Réseaux",
    options: [
      "Télécomunication",
      "Réseau et sécurité informatique",
      "Hygiène sécurité et environnement"
    ]
  },

  "Communication": {
    title: "Communication",
    options: [
      "Journalisme",
      "E-commerce et marketing numérique"
    ]
  },
};


export const actualites = [
    {
      id: "1",
      titre: "Soutenances des rapports de stage en BTS-HND",
      date: "12 Oct, 2024",
      slug: "laureats-ug",
      contenu: "Les soutenances des rapports de fin d’études en BTS, HND se sont récemment tenues à l’Institut Supérieur Rapha (RIH). À cette occasion, les étudiants ont exposé avec succès leurs travaux devant des jurys composés d’experts et d’enseignants, illustrant ainsi leurs compétences et leur engagement. \n\nLes présentations ont couvert une grande diversité de thèmes actuels, touchant des secteurs tels que la conduite de projet, les études de marché, les innovations technologiques et les enjeux du développement durable. Chaque exposé a mis en évidence les savoirs acquis ainsi que l’effort constant fourni durant la période de stage. \n\nL’établissement félicite vivement les étudiants pour la pertinence de leurs travaux et leur attitude professionnelle remarquable. Leur implication et leur ténacité font la renommée de RHI, car ils ont brillamment relevé les exigences du parcours, comme en témoignent la qualité et la clarté de leurs présentations. \n\nL’Institut Supérieur Rapha exprime également sa profonde gratitude à ses partenaires professionnels pour leur accompagnement et leur appui, éléments clés dans le succès de cette étape importante pour les étudiants.",
      images: [images.home24, images.home24, images.home24, images.home24],
    },
    {
      id: "2",
      titre: "Visite officielle de l’Université de Ngaoundéré au campus RHI",
      date: "18 Oct, 2024",
      slug: "partenariat-hopital",
      contenu: "Un nouveau partenariat a été signé avec l’hôpital central pour renforcer...",
      images: [images.home24, images.home24],

    },
    {
      id: "3",
      titre: "Séminaire IA et santé numérique",
      date: "25 Oct, 2024",
      slug: "seminaire-ia",
      contenu: "Ce séminaire a permis aux étudiants de découvrir les enjeux de l’IA en santé...",
      image: images.home24,
    },
    {
      id: "4",
      titre: "Retour en images sur la journée sportive",
      date: "28 Oct, 2024",
      slug: "journee-sportive",
      contenu: "La journée sportive annuelle a été un succès avec une grande participation...",
      image: images.home24,
    },
    {
      id: "5",
      titre: "Atelier sur la cybersécurité",
      date: "30 Oct, 2024",
      slug: "atelier-cyber",
      contenu: "Un atelier de sensibilisation à la cybersécurité a été animé par des experts...",
      image: images.home24,
    },
  ];

 